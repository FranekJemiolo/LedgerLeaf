...
- Name: app.spec.ts >> LedgerLeaf Application >> should display dashboard with summary cards
- Location: tests/app.spec.ts:56:3
...
```yaml
- heading "Error Loading App" [level=2]
...
> 12  |     await expect(page.locator('h1')).toContainText('LedgerLeaf');
  ...
  107 |     
  108 |     // Wait for the modal to close
  109 |     await expect(page.locator('text=Create Expense')).not.toBeVisible();
  110 |     
  111 |     // Check that the expense appears in the table
  112 |     await expect(page.locator('text=Test Subscription')).toBeVisible();
```